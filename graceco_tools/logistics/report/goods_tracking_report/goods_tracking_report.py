...
def execute(filters=None):
	...
	if filters.get("customer"):
        ...
        data = frappe.db.sql("SELECT vsd.delivery_note,dn.customer,vs.vehicle,vs.driver,vsd.delivery_status FROM `tabVehicle Schedule` vs LEFT JOIN `tabVehicle Schedule Delivery` vsd ON (vs.name = vsd.parent) LEFT JOIN `tabDelivery Note` dn ON (vsd.delivery_note = dn.name) WHERE vs.docstatus=1 {0}".format(conditions),filters)
        return columns, data
