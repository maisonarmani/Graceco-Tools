...
def employee_query(doctype, txt, searchfield, start, page_len, filters):
    ...
    sql = """select name,employee_name from `tabEmployee` where (1=1) and ( {cond} {key} like %(txt)s) {mcond} order by if(locate(%(_txt)s, name), locate(%(_txt)s, name), 99999), name limit %(start)s, %(page_len)s""" \
    .format(**{
        "cond":cond,
        'key': searchfield,
