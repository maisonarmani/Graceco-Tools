...
class HelpdeskTicket(Document):
	def validate(self):
		...
		attached_job_cards = frappe.db.sql("SELECT status FROM `tabJob Card` WHERE ticket_number = '{0}'".format(ticket_number))
		...
		if self.status == 'Close':
			if not 'Helpdesk Admin' in frappe.get_roles():
				frappe.throw('Only Helpdesk Admin can close the ticket.')
