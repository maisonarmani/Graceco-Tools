...
def execute(filters=None):
	columns, data = ["Date:Date:200","WPOF No:Link/Weekly Production Order Form:200","Item Code:Link/Item:200","Item Name:Data:200","UOM:Link/UOM:50","Qty:Float:75"], []
	item=""
	item_group=""
	if filters.get("item"):
	...
	data = frappe.db.sql ("""select f.date,f.name,i.item_code,i.item_name,i.uom,i.qty 
		from `tabWeekly Production Order Item` i join `tabWeekly Production Order Form` f on i.parent=f.name 
